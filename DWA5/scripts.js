...
form.addEventListener("submit", (event) => {
  ...
  const dividendInt = parseInt(dividend, 10);                             // Change 1
  const dividerInt = parseInt(divider, 10);

  if (dividend === '' || divider === '') {
      result.innerText = 'Division not performed. Both values are required in inputs. Try again';
  } else if (!isNaN(dividendInt) && !isNaN(dividerInt) && dividerInt > 0) { // Change 2 and Change 5
      const divisionResult = Math.floor(dividendInt / dividerInt);         // Change 3
      result.innerText = divisionResult;
  } else {
      // Change 6: Check for critical error (non-numeric inputs)
      if (isNaN(dividendInt) || isNaN(dividerInt)) {
          document.body.innerHTML = 'Something critical went wrong. Please reload the page';
          console.error('Critical error: Non-numeric input provided.', new Error().stack);
      } else {
          result.innerText = 'Division not performed. Invalid number provided. Try again';
          console.error('Invalid input. The divider should be a positive number.', new Error().stack); // Change 5
      }
  }
});
